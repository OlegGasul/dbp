 ...
 */
@Controller
@RequestMapping("/cylindricalclampingring")
public class CylindricalClampingRingsController extends BaseContainerController<CylindricalClampingRing> {
    ...
    @Autowired
    public CylindricalClampingRingsController(CylindricalClampingRingsService cylindricalClampingRingsService, MessageService messageService) {
        super(cylindricalClampingRingsService, messageService, CylindricalClampingRing.class);
    }
    ...
    public String add(CylindricalClampingRing cylindricalClampingRing, Model model) throws InternalErrorException {
        prepare(cylindricalClampingRing, model);
        return super.addView(cylindricalClampingRing);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(final CylindricalClampingRing cylindricalClampingRing, Model model,
        ...
        validator.validate(cylindricalClampingRing, result);
        if (!result.hasErrors() && CollectionHelper.isNotEmpty(photos)) {
            ...
            try {
                for (MultipartFile file : photos) {
                    if (!file.isEmpty()) {
                        if (!ImageHelper.isImageContentType(file.getContentType())) {
                            result.addError(new FieldError("cylindricalClampingRings", "photos",
                ...
                if (!result.hasErrors() && CollectionHelper.isNotEmpty(photoList)) {
                    cylindricalClampingRing.setPhotos(photoList);
                }
            } catch (IOException e) {
                result.addError(new FieldError("cylindricalClampingRing", "photos", "Failed to attach photos")); //TODO add appropriate message
            }
        }

        if (!result.hasErrors() && !specifications.isEmpty()) {
            try {
                cylindricalClampingRing.setManufacturerSpecifications(specifications.getBytes());
                cylindricalClampingRing.setManufacturerSpecificationsContentType(specifications.getContentType());
                cylindricalClampingRing.setManufacturerSpecificationsFilename(specifications.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("cylindricalClampingRing", "manufacturerSpecificationsFilename",   messageService.getMessage("err.msg.file.attach.failed", "Manufacturer Specifications")));
            }
        }
        if (!result.hasErrors() && !drawing.isEmpty()) {
            try {
                cylindricalClampingRing.setDrawing(drawing.getBytes());
                cylindricalClampingRing.setDrawingContentType(drawing.getContentType());
                cylindricalClampingRing.setDrawingFilename(drawing.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("cylindricalClampingRing", "drawingFilename",  messageService.getMessage("err.msg.file.attach.failed", "Drawing")));
            }
        }
        if (!result.hasErrors() && !drawingCAD.isEmpty()) {
            try {
                cylindricalClampingRing.setDrawingCAD(drawingCAD.getBytes());
                cylindricalClampingRing.setDrawingCADContentType(drawingCAD.getContentType());
                cylindricalClampingRing.setDrawingCADFilename(drawingCAD.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("cylindricalClampingRing", "drawingCADFilename", messageService.getMessage("err.msg.file.attach.failed", "Drawing CAD")));
            }
        }
        ...
        return super.add(cylindricalClampingRing, result, redirectAttributes);
    }
    ...
    public void downloadSpecifications(@PathVariable("id") Long id,
        ...
        CylindricalClampingRing data = modelService.getByID(id);
    ...
    public String edit(@Valid final CylindricalClampingRing cylindricalClampingRing, Model model,
                       BindingResult result,
        ...
        if (specificationsRemoved) {
            cylindricalClampingRing.setManufacturerSpecificationsFilename(null);
        ...
        if (drawingRemoved) {
            cylindricalClampingRing.setDrawingFilename(null);
        ...
        if (drawingCADRemoved) {
            cylindricalClampingRing.setDrawingCAD(null);
        ...
        return super.edit(cylindricalClampingRing, result, redirectAttributes);
    }
    ...
    public void viewPhoto(HttpServletResponse response,
        ...
        CylindricalClampingRingsService service = getService();
    ...
    public String editPhotos(@Valid final CylindricalClampingRing cylindricalClampingRing,
                             @RequestParam(value = "infoPhotos", required = false) List<MultipartFile> photos,
                             BindingResult result,
                             final RedirectAttributes redirectAttributes) throws DataNotFoundException, InternalErrorException {

        return super.editPhotos(cylindricalClampingRing, result, redirectAttributes);
    }
    ...
    @Override
    protected void validate(CylindricalClampingRing cylindricalClampingRing, BindingResult result) {

        if (cylindricalClampingRing.getContainerMaterial() == null || cylindricalClampingRing.getContainerMaterial().getId() == null) {
            result.addError(new FieldError("cylindricalClampingRing", "containerMaterial",
                    messageService.getMessage("err.msg.cylindrical.clamping.ring.container.material.required")));
        }

        if (cylindricalClampingRing.getHole() == null || cylindricalClampingRing.getHole().getId() == null) {
            result.addError(new FieldError("cylindricalClampingRing", "hole",
                    messageService.getMessage("err.msg.cylindrical.clamping.ring.hole.required")));
        }
    }

    private CylindricalClampingRingsService getService() {
        return (CylindricalClampingRingsService) modelService;
    }

    protected void prepare(CylindricalClampingRing cylindricalClampingRing, Model model) throws InternalErrorException {
        prepare(model);
    }
