...
public class HandleController  extends BaseObjectController<Handle> {

    @Autowired
    public HandleController(HandleService handleService, MessageService messageService, LanguageService languageService) {
        super(handleService, messageService, languageService, Handle.class);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.GET)
    public String add(Handle handle, Model model) {
        model.addAttribute("handle", handle);
        return super.addView(handle);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(@Valid final Handle handle,
                      BindingResult result,
                      final RedirectAttributes redirectAttributes) throws InternalErrorException {

        return super.add(handle, result, redirectAttributes);
    }
