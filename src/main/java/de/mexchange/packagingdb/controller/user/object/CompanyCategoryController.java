...
public class CompanyCategoryController extends BaseObjectController<CompanyCategory> {

    @Autowired
    public CompanyCategoryController(CompanyCategoryService companyCategoryService, MessageService messageService, LanguageService languageService) {
        super(companyCategoryService, messageService, languageService, CompanyCategory.class);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.GET)
    public String add(CompanyCategory companyCategory, Model model) {
        model.addAttribute("companycategory", companyCategory);
        return super.addView(companyCategory);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(@Valid final CompanyCategory companyCategory,
                      BindingResult result,
                      final RedirectAttributes redirectAttributes) throws InternalErrorException {

        return super.add(companyCategory, result, redirectAttributes);
    }
    ...
    public String edit(@Valid final CompanyCategory companyCategory,
                       BindingResult result,
                       final RedirectAttributes redirectAttributes) throws DataNotFoundException, InternalErrorException {

        return super.edit(companyCategory, result, redirectAttributes);
    }
