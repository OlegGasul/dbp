...
public class TransportController extends BaseObjectController<Transport> {

    @Autowired
    public TransportController(TransportService transportService , MessageService messageService, LanguageService languageService) {
        super(transportService, messageService, languageService, Transport.class);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.GET)
    public String add(Transport transport, Model model) {
        model.addAttribute("transport", transport);
        return super.addView(transport);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(@Valid final Transport transport,
                      BindingResult result,
                      final RedirectAttributes redirectAttributes) throws InternalErrorException {

        return super.add(transport, result, redirectAttributes);
    }
    ...
    public String edit(@Valid final Transport transport,
                       BindingResult result,
                       final RedirectAttributes redirectAttributes) throws DataNotFoundException, InternalErrorException {

        return super.edit(transport, result, redirectAttributes);
    }
