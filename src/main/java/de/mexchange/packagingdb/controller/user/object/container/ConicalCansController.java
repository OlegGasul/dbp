 ...
 */
@Controller
@RequestMapping("/conicalcans")
public class ConicalCansController extends BaseContainerController<ConicalCans> {
    ...
    @Autowired
    public ConicalCansController(ConicalCansService conicalCansService, MessageService messageService) {
        super(conicalCansService, messageService, ConicalCans.class);
    }
    ...
    public String add(ConicalCans conicalCans, Model model) throws InternalErrorException {
        prepare(conicalCans, model);
        return super.addView(conicalCans);
    }

    @PreAuthorize("hasRole('ROLE_WRITER') or hasRole('ROLE_ADMIN')")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(final ConicalCans conicalCans, Model model,
        ...
        validator.validate(conicalCans, result);
        if (!result.hasErrors() && CollectionHelper.isNotEmpty(photos)) {
            ...
            try {
                for (MultipartFile file : photos) {
                    if (!file.isEmpty()) {
                        if (!ImageHelper.isImageContentType(file.getContentType())) {
                            result.addError(new FieldError("conicalCans", "photos",
                ...
                if (!result.hasErrors() && CollectionHelper.isNotEmpty(photoList)) {
                    conicalCans.setPhotos(photoList);
                }
            } catch (IOException e) {
                result.addError(new FieldError("conicalCans", "photos", "Failed to attach photos")); //TODO add appropriate message
            }
        }

        if (!result.hasErrors() && !specifications.isEmpty()) {
            try {
                conicalCans.setManufacturerSpecifications(specifications.getBytes());
                conicalCans.setManufacturerSpecificationsContentType(specifications.getContentType());
                conicalCans.setManufacturerSpecificationsFilename(specifications.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("conicalCans", "manufacturerSpecificationsFilename",   messageService.getMessage("err.msg.file.attach.failed", "Manufacturer Specifications")));
            }
        }
        if (!result.hasErrors() && !drawing.isEmpty()) {
            try {
                conicalCans.setDrawing(drawing.getBytes());
                conicalCans.setDrawingContentType(drawing.getContentType());
                conicalCans.setDrawingFilename(drawing.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("conicalCans", "drawingFilename",  messageService.getMessage("err.msg.file.attach.failed", "Drawing")));
            }
        }
        if (!result.hasErrors() && !drawingCAD.isEmpty()) {
            try {
                conicalCans.setDrawingCAD(drawingCAD.getBytes());
                conicalCans.setDrawingCADContentType(drawingCAD.getContentType());
                conicalCans.setDrawingCADFilename(drawingCAD.getOriginalFilename());
            } catch (IOException e) {
                result.addError(new FieldError("conicalCans", "drawingCADFilename", messageService.getMessage("err.msg.file.attach.failed", "Drawing CAD")));
            }
        }
        ...
        return super.add(conicalCans, result, redirectAttributes);
    }
    ...
    public void downloadSpecifications(@PathVariable("id") Long id,
        ...
        ConicalCans data = modelService.getByID(id);
    ...
    public String edit(@Valid final ConicalCans conicalCans, Model model,
                       BindingResult result,
        ...
        if (specificationsRemoved) {
            conicalCans.setManufacturerSpecificationsFilename(null);
        ...
        if (drawingRemoved) {
            conicalCans.setDrawingFilename(null);
        ...
        if (drawingCADRemoved) {
            conicalCans.setDrawingCAD(null);
        ...
        return super.edit(conicalCans, result, redirectAttributes);
    }
    ...
    public void viewPhoto(HttpServletResponse response,
        ...
        ConicalCansService service = getService();
    ...
    public String editPhotos(@Valid final ConicalCans conicalCans,
                             @RequestParam(value = "infoPhotos", required = false) List<MultipartFile> photos,
                             BindingResult result,
                             final RedirectAttributes redirectAttributes) throws DataNotFoundException, InternalErrorException {

        return super.editPhotos(conicalCans, result, redirectAttributes);
    }
    ...
    @Override
    protected void validate(ConicalCans conicalCans, BindingResult result) {


        if (conicalCans.getContainerMaterial() == null || conicalCans.getContainerMaterial().getId() == null) {
            result.addError(new FieldError("conicalCans", "containerMaterial",
                    messageService.getMessage("err.msg.conical.cans.container.material.required")));
        }

        if (conicalCans.getHole() == null || conicalCans.getHole().getId() == null) {
            result.addError(new FieldError("conicalCans", "hole",
                    messageService.getMessage("err.msg.conical.cans.hole.required")));
        }
    }

    private ConicalCansService getService() {
        return (ConicalCansService) modelService;
    }

    protected void prepare(ConicalCans conicalCans, Model model) throws InternalErrorException {
        prepare(model);
    }
